...
const ADD_USER_OWNED_TEAMS = "teams/ADD_USER_OWNED_TEAMS";
...
const addUserOwnerTeams = (teams) => ({
  type: ADD_USER_OWNED_TEAMS,
  payload: teams,
});

export const getUserOwnedTeamsThunk = () => async (dispatch) => {
  const response = await fetch("/api/teams/current");
  if (response.ok) {
    const teams = await response.json();
    dispatch(addUserOwnerTeams(teams));
    return null;
  } else {
...
// Teams reducer
const initialState = { allTeams: {}, currentTeam: null, userTeams: [], userOwnerTeams: [] };
const teamsReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case ADD_USER_OWNED_TEAMS: {
        const newState = { ...state, userOwnedTeams: [] };
        action.payload.forEach((team) => {
          newState.userOwnedTeams[team.id] = team;
        });
        return newState;
    }
    default:
